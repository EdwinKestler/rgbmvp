...
| **T1** | Bounded public demo swaps | **In tree, OFF by default, never deployed** — [TESTNET_PUBLIC_SWAPS.md](./TESTNET_PUBLIC_SWAPS.md). Adds ONE public mutation (`POST /v1/demo/swap`) behind Turnstile + quotas + fee budget + custody preflight. Deliberately narrows ADR-U4: the service holds spendable **testnet** keys when enabled. No live swap has been executed through it yet. |
